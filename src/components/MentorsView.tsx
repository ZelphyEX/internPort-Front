...
/**
 * Tab "Mentor" — chỉ Quản trị viên (ADMIN) thấy, và CHỈ liệt kê tài khoản Mentor
 * (khác tab "Thực tập sinh", nơi chỉ liệt kê Intern — hai tab không hiện chung một
 * danh sách gộp cả hai vai trò). Ba khối, xếp theo mức cần xử lý:
 *
 ...
 *   3. **Danh sách Mentor** — trình bày giống trang Thực tập sinh (thẻ / bảng, có ô
 *      tìm). Mỗi dòng có nút "Chuyển thành Thực tập sinh" để Admin tự tay hạ quyền
 *      mà không cần người đó xin trước; chiều ngược lại (Intern lên Mentor) nằm ở
 *      nút tương ứng bên tab "Thực tập sinh".
 *
...
/**
 * Lấy TOÀN BỘ tài khoản Mentor, đi hết các trang.
 *
 * Backend chặn `size > 100` (422); nếu số Mentor vượt quá đó, lấy đúng 100 người
 * đầu rồi thôi thì những người sau lặng lẽ biến mất khỏi giao diện, không có dấu
 * hiệu gì báo là bị cắt. Nên đọc trang 1 để biết tổng số trang rồi tải nốt phần
 * còn lại.
 */
async function fetchAllMentors(): Promise<ApiUser[]> {
  const first = await usersApi.list({ page: 1, size: 100, role: 'MENTOR' });
  if (first.pages <= 1) return first.items;

  const rest = await Promise.all(
    Array.from({ length: first.pages - 1 }, (_, i) =>
      usersApi.list({ page: i + 2, size: 100, role: 'MENTOR' }).then((r) => r.items)
    )
  );
...
export const MentorsView: React.FC<MentorsViewProps> = ({ currentRole, onQueueChanged }) => {
  ...
  const load = () => {
    ...
    setLoading(true);
    Promise.all([
      fetchAllMentors().catch(() => [] as ApiUser[]),
  ...
  // --- Admin tự tay hạ quyền xuống Thực tập sinh --------------------------- #
  // Khác khối "Yêu cầu chuyển vai trò" ở trên: ở đó Admin DUYỆT cái người dùng đã
  // xin; ở đây Admin CHỦ ĐỘNG hạ quyền, người kia không cần xin gì cả. Chiều
  // ngược lại (Intern lên Mentor) nằm ở nút tương ứng trong InternsView, vì trang
  // này chỉ liệt kê Mentor.
  const handleDemoteToIntern = async (u: ApiUser) => {
    if (
      !window.confirm(
        `Chuyển "${u.full_name}" từ Mentor xuống Thực tập sinh?\n\n` +
          'Sau khi chuyển, người này MẤT toàn bộ quyền quản lý (không còn gán lộ trình, ' +
          'giao task hay duyệt báo cáo được nữa). Dữ liệu đã tạo trước đó vẫn giữ nguyên, ' +
          'không bị xoá.\n\n' +
          'Vai trò đổi ngay lập tức. Bạn có thể chuyển ngược lại ở tab "Thực tập sinh".'
      )
    ) {
      return;
    }
    ...
    setBusyId(u.id);
    try {
      await usersApi.setRole(u.id, 'INTERN');
      afterMutation();
    } catch (err) {
  ...
  // Danh sách chính: chỉ Mentor, lọc theo ô tìm + trạng thái.
  const filtered = all
  ...
  const pending = all.filter((u) => u.status === 'PENDING' && matchesSearch(u)).sort((a, b) => a.id - b.id);
  ...
  /** Nút hạ quyền — dùng chung cho thẻ và bảng. */
  const renderDemoteButton = (u: ApiUser, compact: boolean) => {
    ...
    return (
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
          handleDemoteToIntern(u);
        }}
        title="Chuyển thành Thực tập sinh"
        className={`inline-flex items-center gap-1.5 rounded-xl font-bold transition-colors cursor-pointer disabled:opacity-50 bg-slate-100 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 ${
      ...
      >
        ...
        <span>Chuyển thành Thực tập sinh</span>
      </button>
    );
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* 3. Danh sách Mentor — cùng bố cục với trang Thực tập sinh, nhưng CHỈ
              chứa Mentor (xem lời giải thích ở đầu file: hai tab không gộp chung). */}
          <div className="space-y-3">
            <h3 className="text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span>Mentor đang hoạt động ({filtered.length})</span>
            </h3>
                      ...
                      {filtered.map((u) => (
                        <tr key={u.id} className="hover:bg-blue-50/40 dark:hover:bg-slate-700/30 transition-colors">
                          ...
                          <td className="p-4 text-right">
                            <div className="flex items-center justify-end">{renderDemoteButton(u, false)}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <p className="text-[11px] text-slate-400 flex items-start gap-1.5">
              ...
              <span>
                Muốn đưa một Thực tập sinh lên làm Mentor? Dùng nút tương ứng ở tab
                "Thực tập sinh" — mỗi tab chỉ quản lý vai trò của chính nó.
              </span>
            </p>
          </div>
        </>
